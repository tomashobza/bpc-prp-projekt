...
TEST(KinematicsTest, BackwardPositiveLinearVelocitySI) {
    ...
    const float angular = 0;
    const float expected_l = 1.0 / WHEEL_RADIUS;  
    const float expected_r = 1.0 / WHEEL_RADIUS;
...
TEST(KinematicsTest, BackwardPositiveAngularVelocitySI) {
    ...
    const float expected_l = -(0.5 * WHEEL_BASE) / WHEEL_RADIUS;
    const float expected_r = +(0.5 * WHEEL_BASE) / WHEEL_RADIUS;
...
TEST(KinematicsTest, ForwardOppositeWheelSpeedsSI) {
    ...
    const float wheel_r = 1;
    const float expected_l = 0;
    const float expected_a = 0.55;  // This is the actual value calculated by the kinematics formula
